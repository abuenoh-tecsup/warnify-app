...
<div data-id="{{ $reporteId }}" class="max-w mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-2xl flex mb-3">
    <div class="p-3 bg-blue w-5/6 {{ $attributes->get('class', '') }}">
    ...
    <a class="w-1/6 bg-blue-300 hover:bg-blue-400 focus:outline-none focus:ring-2 focus:ring-yellow-500 flex items-center justify-center" href="{{ route('reportes.list', ['filter' => request('filter', 'all'), 'state' => request('state'), 'order' => request('order', 'desc'), 'id' => $reporteId]) }}">
